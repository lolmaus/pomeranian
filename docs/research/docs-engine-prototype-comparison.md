...
The throwaway code is retained on local branch `prototype/docs-engine-comparison`,
under `prototypes/docs-engines/`. The worktree used here is
`/tmp/pomeranian-docs-engine-prototypes`; the committed branch preserves the
inputs independently of disposable dependencies, output, and caches. It has not
been pushed. The main checkout contains research and roadmap updates only.

In that worktree, select the pinned Node/pnpm versions and run:
