...
This private Playwright Test suite consumes
`@pomeranian/lib-essential/element-po` against the independent React fixture.
...
```sh
pnpm exec playwright test tests/counter.spec.ts
```

The full `test` command additionally verifies named steps and diagnostics in the
genuine JSON report. Matching edge cases catch failures and check their specific
...
files receive test-structure lint rules; `shouldHaveText` and
`shouldNotHaveText` are registered assertion methods, while `click` is not.
